...
//这是一个私有类，用于将 SVG 元素视为按钮.
//如果我们需要一个通用的 SVG 按钮，我们可以将其设为通用的.
 ...
 * <br /><br />
 * "穿梭环"概念借鉴自视频编辑,通常可以旋转"转盘"以非常缓慢地移动单个动画帧,并且可以扭转周围的穿梭环来控制快速播放的方向和速度.
 * Cesium 通常将时间视为连续的（不分解为预定义的动画帧）,因此此小部件不提供转盘.相反,穿梭环能够快速和非常慢速播放.单击并拖动穿梭环指针本身（如上图所示，以绿色显示),
 * 或单击环区域的其余部分以将指针轻推到该方向的下一个预设速度.
 * <br /><br />
 * 动画小部件还提供了一个"实时"按钮（位于左上角）,可使动画时间与最终用户的系统时钟保持同步，通常显示"今天"或"现在".
 * 如果当前事件超出{@link Clock}的开始时间和结束时间,则此模式在{@link ClockRange.CLAMPED}或{@link ClockRange.LOOP_STOP}模式下不可用.
 *
 * @alias Animation
 * @constructor
 *
 * @param {Element|string} container 将包含 小部件 的 DOM 元素或 ID.
...
/**
 * @returns {boolean} 如果对象已被销毁，则返回 true,否则返回 false.
 */
