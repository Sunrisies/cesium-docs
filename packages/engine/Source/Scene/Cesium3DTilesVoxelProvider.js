...
/**
 * 一个 {@link VoxelProvider}，用于从 3D Tiles 瓦片集中获取体素数据。
 * <p>
 * 实现了 {@link VoxelProvider} 接口。
 * </p>
 * <div class="notice">
 * 此对象通常不是直接实例化的，使用 {@link Cesium3DTilesVoxelProvider.fromUrl}。
 * </div>
 *
...
/**
 * 创建一个 {@link VoxelProvider}，用于从 3D Tiles 瓦片集中获取体素数据。
 *
 * @param {Resource|string} url 瓦片集 JSON 文件的 URL
 * @returns {Promise<Cesium3DTilesVoxelProvider>} 创建的提供者
 *
 * @exception {RuntimeException} 根必须具有内容
 * @exception {RuntimeException} 根瓦片内容必须具有 3DTILES_content_voxels 扩展
 * @exception {RuntimeException} 根瓦片必须具有隐式分块
 * @exception {RuntimeException} 瓦片集必须具有元数据架构
 * @exception {RuntimeException} Cesium3DTilesVoxelProvider 仅支持框、区域和 3DTILES_bounding_volume_cylinder
 */
